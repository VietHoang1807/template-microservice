...
public class DateCommon {
    ...
    public static Date convertDateToDate(Date date) {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat(Constant.DATE.FORMAT.DATE);
        return convertStringToDate(sdf.format(date));
    }
    ...
    public static int dayEndOfMonth(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
    ...
    public static boolean differenceBetween(Date p1, Date p2) {
        if (p1 == null || p2 == null) return true;
        return p1.toInstant().atZone(ZoneId.systemDefault()).toLocalDate()
                .isBefore(p2.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
    }
    public static boolean differenceBetween(Date p1, Date p2, int conf) {
        if (p1 == null || p2 == null) return true;
        return (p1.toInstant().atZone(ZoneId.systemDefault()).isBefore(p2.toInstant().atZone(ZoneId.systemDefault()))
                    || p1.toInstant().atZone(ZoneId.systemDefault()).equals(p2.toInstant().atZone(ZoneId.systemDefault())));
    }

    public static boolean differenceInCalendarMonths(Date p1, Date p2, int numb) {
        int m1 = yearOfDate(p1) * 12 + monthOfDate(p1); // đến ngày
        int m2 = yearOfDate(p2) * 12 + monthOfDate(p2); // từ ngày
        int calMonths = m1 - m2; // đến ngày trừ từ ngày lớn hơn bằng numb
        if (calMonths == numb) {
            int ed1 = dayEndOfMonth(p1);
            int d1 = dayOfMonth(p1);
            int ed2 = dayEndOfMonth(p2);
            int d2 = dayOfMonth(p2);
            if (d1 <= ed1 && d2 == ed2) return false;
            return d1 > d2;
        }
        return calMonths > numb;
    }
    public static boolean differenceInCalendarDays(Date p1, Date p2, int numb) {
        return daysBetween(p1, p2) <= numb;
    }

    public static long daysBetween(Date p1, Date p2) {
        return ChronoUnit.DAYS.between(p1.toInstant(), p2.toInstant());
    }
}
